...
web3js=new Web3(new Web3.providers.HttpProvider("http://localhost:8545"))}});web3.eth.getAccounts(function(err,accounts){if(err!=null)console.error("An error occurred: "+err);else if(accounts.length==0){console.log("User is not logged in to MetaMask");document.getElementById('warning').innerHTML="Please log into MetaMask"}
else console.log("User is logged in to MetaMask")});web3.version.getNetwork((err,netId)=>{switch(netId){case "1":console.log('This is mainnet')
